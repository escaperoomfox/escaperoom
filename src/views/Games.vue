    ...
    >
      ...
      <div class="quest">
        <button
          ...
          v-for="(answer, i) in allQuestions[index].answer"
          :key="i"
          type="button"
          :id="`${i}-${index}`"
...
import song from "@/assets/Moment-of-Strange.mp3"
...
export default {
  ...
  created() {
    ...
    this.$store
      .dispatch("getQuestions", this.$route.params.id)
      .then(() => {
  ...
  methods: {
    ...
    chosen(data, i) {
      let pos = data.answer.indexOf(data.correct_answer);
      if (data.correct_answer === data.answer[i]) {
        console.log("bener");
